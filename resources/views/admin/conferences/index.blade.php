...
<div class="table-responsive mt-4" style="font-family: Arial, sans-serif;">
    <h2 class="mb-3" style="color: #027c7d; font-weight: bold; font-size: large;">ALL CONFERENCES</h2>
        ...
        <tbody style="color: #000120;">
            @forelse ($conferences as $index => $conference)
                <tr>
                    ...
                    <td class="py-2 px-3">{{ $conference->category->name ?? '-' }}</td>
                    <td class="py-2 px-3">{{ $conference->topic->name ?? '-' }}</td>
                    <td class="py-2 px-3">{{ \Str::limit($conference->description, 50) }}</td>
                    <td class="py-2 px-3">
                        @if ($conference->paper_path)
                            <a href="{{ asset('storage/' . $conference->paper_path) }}" target="_blank" class="text-primary">View Paper</a>
                        @else
                            <span class="text-muted">N/A</span>
                        @endif
                    </td>
                    <td class="py-2 px-3">
                        <small>{{ $conference->publication_date ?? '-' }}</small>
                    </td>
                    <td class="py-2 px-3">{{ $conference->contact_email }}</td>
                    <td class="py-2 px-3">
            ...
            @empty
                <tr>
                    <td colspan="9" class="text-center" style="color: #000120;">No conferences found.</td>
                </tr>
            @endforelse
        </tbody>
    </table>
