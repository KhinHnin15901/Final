...
<section class="section d-flex justify-content-center" style="font-family: Arial, sans-serif; padding: 30px;">
    <div class="card" style="max-width: 900px; width: 100%; padding: 24px; background: #fff; border-radius: 12px; box-shadow: 0 4px 15px rgba(214, 221, 66, 0.6); border-top: 4px solid #027c7d;">
        ...
        <h4 class="text-center mb-5 fw-bold" style="color: #000120; font-size: 22px;">
            ✏️ Edit Conference Review Schedule
        </h4>

        <form method="POST" action="{{ route('admin.schedule.conferenceupdate', $schedules->id) }}" enctype="multipart/form-data" class="row g-3">
            @csrf
            @method('PUT')
            ...
            <div class="col-12">
                ...
                <input type="text" class="form-control" value="{{ $schedules->conferenceSubmission->title ?? \Illuminate\Support\Str::of($schedules->conferenceSubmission->paper_path)->afterLast('/') }}" readonly>
            </div>
