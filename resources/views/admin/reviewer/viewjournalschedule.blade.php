...
<div class="table-responsive mt-8" style="font-family: Arial, sans-serif;">
    <h2 class="mb-3" style="color: #027c7d; font-weight: bold; font-size: large;">📅 Journal Review Schedule List</h2>
            ...
            @forelse ($schedules as $index => $schedule)
                <tr>
                    ...
                    <td class="py-2 px-3 text-nowrap">
                        <!-- Edit Button -->
                        <a href="{{ route('admin.schedule.journaledit', $schedule->id) }}" class="btn btn-warning btn-sm me-1">
                        ...
                        @if($schedule->status === 'send')
                            ...
                            <form id="unsend-form-{{ $schedule->id }}" action="{{ route('admin.schedule.journalsend', $schedule->id) }}" method="POST" style="display: none;">
                                @csrf
                                @method('PUT')
                            </form>
                        @else
                            ...
                            <form id="send-form-{{ $schedule->id }}" action="{{ route('admin.schedule.journalsend', $schedule->id) }}" method="POST" style="display: none;">
                                @csrf
                                @method('PUT')
                            </form>
                        @endif
                        ...
                        <form id="delete-form-{{ $schedule->id }}" action="{{ route('admin.schedule.journaldestroy', $schedule->id) }}" method="POST" style="display: none;">
                            @csrf
                            @method('DELETE')
                        </form>
                    </td>
