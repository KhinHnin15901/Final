...
@section('main-content')
    <div
        ...
        <h2 class="text-4xl font-extrabold text-center text-gray-800 mb-10 tracking-tight">
            📅Conference Review Schedule List
        </h2>
                ...
                @forelse ($schedules as $index => $schedule)
                    <tr class="border-t hover:bg-gray-50 transition">
                        ...
                        <td class="px-6 py-2 text-center">
                            <!-- Edit Button -->
                            <a href="{{ route('admin.schedule.conferenceedit', $schedule->id) }}"
                            ...
                            @if ($schedule->status === 'send')
                                ...
                                <form id="unsend-form-{{ $schedule->id }}"
                                    action="{{ route('admin.schedule.conferencesend', $schedule->id) }}" method="POST"
                                    style="display: none;">
                                    @csrf
                                    @method('PUT')
                                </form>
                            ...
                            <form id="delete-form-{{ $schedule->id }}"
                                action="{{ route('admin.schedule.conferencedestroy', $schedule->id) }}" method="POST"
                                style="display: none;">
                                @csrf
                                @method('DELETE')
                            </form>
