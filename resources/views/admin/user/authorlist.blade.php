        ...
        <tbody style="color: #000120;">
            @forelse ($authors as $index => $author)
                <tr>
                    ...
                    <td class="py-2 px-3">{{ $author->name ?? '-' }}</td>
                    <td class="py-2 px-3">{{ $author->email ?? '-' }}</td>
                    <td class="py-2 px-3">{{ $author->position ?? '-' }}</td>
                    <td class="py-2 px-3">{{ $author->department ?? '-' }}</td>
                    <td class="py-2 px-3">{{ $author->organization ?? '-' }}</td>
                    <td class="text-nowrap py-2 px-3">
                        ...
                        <a href="#"
                            onclick="event.preventDefault(); if(confirm('Are you sure you want to delete this author?')) { document.getElementById('delete-form-{{ $author->id }}').submit(); }"
            ...
            @empty
                <tr>
                    <td colspan="8" class="text-center" style="color: #000120;">No author found.</td>
                </tr>
            @endforelse
        </tbody>
    </table>
